...
// ClientOption configures the software-download SDK transport at construction
// time. Pass one or more ClientOption values to NewClient.
type ClientOption = client.ClientOption

// WithTimeout sets a custom timeout for HTTP requests.
// Defaults to 2 minutes. Downloads override this per-request (no timeout).
...
// WithUserAgent sets a custom user-agent string. Note: the service layer sends a
// browser user-agent per request regardless, since Microsoft's download flow
// rejects non-browser agents; this option affects only other requests.
...
// WithTLSClientConfig sets custom TLS configuration. When nil the transport uses
// the system certificate pool. Ignored when WithInsecureSkipVerify is set.
