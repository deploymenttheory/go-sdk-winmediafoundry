...
//
// The production implementation lives in wuproto/soap. Tests substitute
// wuproto/mock.MockWindowsUpdateClient.
package wuproto
...
type WindowsUpdateClient interface {
	...
	FetchUpdates(ctx context.Context, req FetchRequest) ([]UpdateResult, error)
	...
	GetFileURLs(ctx context.Context, req FileURLRequest) ([]FileURL, error)
}
...
type FetchRequest struct {
	...
	// build (e.g. "10.0.16251.0") to receive the current stable release as an
	// upgrade. Defaults to "10.0.16251.0" when empty.
	CheckBuild string
...
const (
	...
	RingWIF Ring = "WIF" // Windows Insider Fast  → Dev
