...
const (
	...
	// userAgent matches the Windows Update Agent used by the PHP implementation.
...
func newCookieManager(hc *http.Client, logger *zap.Logger) (*cookieManager, error) {
...
// cookie returns a valid cookie, refreshing it if necessary.
...
func (m *cookieManager) acquire(ctx context.Context) (*cachedCookie, error) {
	...
	if resp.StatusCode != http.StatusOK {
		...
		return nil, fmt.Errorf("GetCookie returned HTTP %d: %s", resp.StatusCode, string(raw))
	}

	raw, err := io.ReadAll(resp.Body)
	if err != nil {
		return nil, fmt.Errorf("read GetCookie response: %w", err)
	}
...
// post performs an HTTP POST with the Windows Update SOAP content type.
func (m *cookieManager) post(ctx context.Context, url, action, body string) (*http.Response, error) {
